...
const float songLength = 108.0; // define song length in seconds
...
void main() {
    ...
    for (int r = 0; r < volsteps; r++) {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            a += abs(length(p) - pa / (0.5 * abs(vPos.y) / ((u_time / songLength + 0.1)) * 2.0)); // add abs() to prevent negative values
