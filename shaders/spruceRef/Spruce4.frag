...
void main() {
    ...
    float chaosA = sin(u_time * 0.1 + vPos.y * 2.1) * 1.2;
    float chaosB = cos(u_time * 0.2 + vPos.x * 1.7) * 1.5;
    float chaosC = sin(vPos.x * 0.5 + vPos.y * 0.3 + u_time * 0.15) * 2.0;
    ...
    float zoomMod = 1.0 + 0.1 * sin(u_time * 0.8 + length(vPos.xy));
    dir *= zoomMod;
    ...
    for (int r = 0; r < volsteps; r++) {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            a += abs(length(p) - pa / (2.0 * abs(vPos.y) / ((u_time / songLength + 0.1)) / 10.0));
