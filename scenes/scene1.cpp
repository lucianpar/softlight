...
public:
  ...
  void onCreate() override {
    ...
    for (int i = 0; i < bodyMesh.vertices().size(); ++i) {
      bodyMesh.color(1.0, 0.6, 0.3, startingBodyAlpha);
      bodyMesh.update();
      ...
      for (int i = 0; i < attractorMesh.vertices().size(); ++i) {
        attractorMesh.color(1.0, 0.6, 0.2, 0.7);
      }
        ...
        if (sceneTime >= bodyCloudAppear) {
          if (bodyAlphaIncScene1 < 1.0) {
            ...
            for (int i = 0; i < bodyMesh.vertices().size(); ++i) {
              ...
              currentColor.a = startingBodyAlpha +
                               bodyAlphaIncScene1; // Increment alpha only
